...
// 4. Add the following CSS import to your global stylesheet (`app/globals.css`):
...
// NOTE: react-quill and its CSS are now loaded from a CDN to avoid build issues.
...
// Extend window type to include katex and ReactQuill from the CDN
declare global {
  interface Window {
    katex: any;
    ReactQuill: ComponentType<any>;
  }
}

const EditorLoading = () => <div className="p-4 text-center border rounded-md bg-gray-50 h-32 flex items-center justify-center">Loading Editor...</div>;

// Wrapper component to load ReactQuill from a CDN at runtime.
// This completely avoids build-time module resolution errors.
    ...
    useEffect(() => {
        if (typeof window !== 'undefined') {
            ...
            if (window.ReactQuill) {
                setEditor(() => window.ReactQuill);
                return;
            }

            const css = document.createElement('link');
            css.rel = 'stylesheet';
            css.href = 'https://unpkg.com/react-quill@2.0.0/dist/quill.snow.css';
            document.head.appendChild(css);

            const script = document.createElement('script');
            script.src = 'https://unpkg.com/react-quill@2.0.0/dist/react-quill.js';
            script.async = true;
            script.onload = () => {
                if (window.ReactQuill) {
                    setEditor(() => window.ReactQuill);
...
export default function CreateQuizPage() {
  ...
  const QuestionForm: React.FC<{ onSave: (question: Question) => void }> = ({ onSave }) => {
    ...
    const [correctOptionIndex, setCorrectOptionIndex] = useState<string | null>(null);
    ...
    return (
      <div className="grid gap-6 py-4">
        <div>
          <Label htmlFor="question" className="text-lg font-semibold mb-2 block">Question</Label>
        ...
        <div>
           ...
           <RadioGroup value={correctOptionIndex ?? undefined} onValueChange={setCorrectOptionIndex}>
        ...
        <Button variant="outline" onClick={handleAddOption} disabled={options.length >= 5} className="w-full">
            <PlusCircle className="mr-2 h-4 w-4" /> Add Option
        </Button>
        <DialogFooter>
          <Button onClick={handleSubmit} className="w-full sm:w-auto">Save Question</Button>
        </DialogFooter>
      </div>
    );
  };
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <DialogContent className="sm:max-w-4xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>
                Use the editor to create your question and options. Use the 'fx' button for mathematical formulas.
              </DialogDescription>
            </DialogHeader>
      ...
      <main>
        {questions.length === 0 ? (
            <div className="text-center py-16 px-8 bg-white rounded-lg shadow-md">
                <h2 className="text-2xl font-semibold text-gray-700 mb-2">No Questions Yet!</h2>
                <p className="text-gray-500">Click "Add Question" to start building your quiz.</p>
            </div>
        ) : (
                    ...
                    <div className="space-y-3">
                        {q.options.map((opt, optIndex) => (
                           <div
                            ...
                            >
                                <div className={`w-6 h-6 rounded-full flex items-center justify-center text-sm font-bold text-white ${ optIndex === q.correctAnswer ? 'bg-green-500' : 'bg-gray-400'}`}>
